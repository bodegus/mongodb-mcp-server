...
exports.FindArgs = {
    ...
    sort: zod_1.z
        .record(zod_1.z.string(), mongodbSchemas_js_1.SortDirectionSchema.describe("The sort key and its direction"))
        .optional()
            ...
            const [queryResultsCount, cursorResults] = await Promise.all([
                (0, operationWithFallback_js_1.operationWithFallback)(() => provider.countDocuments(database, collection, filter, {
                    ...
                    maxTimeMS: this.config.maxTimeMS !== undefined
                        ? Math.min(this.config.maxTimeMS, constants_js_1.QUERY_COUNT_MAX_TIME_MS_CAP)
                        : constants_js_1.QUERY_COUNT_MAX_TIME_MS_CAP,
                    signal,
                }), undefined),
