...
const ResponseFormat = zod_1.z.enum(["concise", "detailed"]);
class StreamsDiscoverTool extends streamsToolBase_js_1.StreamsToolBase {
    constructor() {
        super(...arguments);
        this.description = "Discover and inspect Atlas Stream Processing resources. " +
        ...
        this.argsShape = {
            ...
            action: DiscoverAction.describe("What to look up. Start with 'list-workspaces' to see available workspaces, " +
            ...
            resourceName: zod_1.z
                .string()
                .optional()
                .describe("Connection or processor name. Required for 'inspect-connection', 'inspect-processor', and 'diagnose-processor'."),
            responseFormat: ResponseFormat.optional().describe("Response detail level. 'concise' returns names and states only. " +
                "'detailed' returns full configuration and stats. " +
                "Default: 'concise' for list actions, 'detailed' for inspect/diagnose."),
            cloudProvider: zod_1.z
                .string()
                .optional()
                .describe("Cloud provider (AWS, AZURE, GCP). Only for 'get-networking': returns account details for the specified provider."),
            region: zod_1.z
                .string()
                .optional()
            ...
            pageNum: zod_1.z.number().int().min(1).optional().describe("Page number for list actions. Default: 1."),
        };
    }
    async execute({ projectId, action, workspaceName, resourceName, responseFormat, cloudProvider, region, limit, pageNum, }) {
        switch (action) {
            ...
            case "list-connections":
                return this.listConnections(projectId, this.requireWorkspaceName(workspaceName), responseFormat, limit, pageNum);
            case "inspect-connection":
                return this.inspectConnection(projectId, this.requireWorkspaceName(workspaceName), this.requireResourceName(resourceName, "connection"));
            case "list-processors":
                return this.listProcessors(projectId, this.requireWorkspaceName(workspaceName), responseFormat, limit, pageNum);
            case "inspect-processor":
                return this.inspectProcessor(projectId, this.requireWorkspaceName(workspaceName), this.requireResourceName(resourceName, "processor"));
            case "diagnose-processor":
                return this.diagnoseProcessor(projectId, this.requireWorkspaceName(workspaceName), this.requireResourceName(resourceName, "processor"));
            case "get-networking":
    ...
    requireWorkspaceName(workspaceName) {
        if (!workspaceName) {
            throw new Error("workspaceName is required for this action. Use action 'list-workspaces' to see available workspaces.");
        }
        return workspaceName;
    }
    requireResourceName(resourceName, resourceType) {
        if (!resourceName) {
            throw new Error(`resourceName is required to inspect a ${resourceType}. ` +
                `Use 'list-${resourceType}s' action to see available ${resourceType}s.`);
        }
        return resourceName;
    }
    async listWorkspaces(projectId, responseFormat, limit, pageNum) {
        ...
        const workspaces = format === "concise"
            ? data.results.map((w) => ({
                name: w.name,
            ...
            : data.results;
        return {
            content: (0, tool_js_1.formatUntrustedData)(`Found ${data.results.length} workspace(s) (total: ${data.totalCount ?? data.results.length}):`, JSON.stringify(workspaces, null, 2)),
        };
    }
    async inspectWorkspace(projectId, workspaceName, responseFormat) {
        ...
        const output = format === "concise"
            ...
            : data;
        return {
            content: (0, tool_js_1.formatUntrustedData)(`Workspace '${workspaceName}' details:`, JSON.stringify(output, null, 2)),
        };
    }
    async listConnections(projectId, workspaceName, responseFormat, limit, pageNum) {
        ...
        const format = responseFormat ?? "concise";
        const connections = format === "concise"
            ? data.results.map((c) => {
                const conn = c;
                return {
                    name: conn.name,
            ...
            : data.results;
        return {
            content: (0, tool_js_1.formatUntrustedData)(`Found ${data.results.length} connection(s) in workspace '${workspaceName}':`, JSON.stringify(connections, null, 2)),
        };
    }
    async inspectConnection(projectId, workspaceName, connectionName) {
        ...
        return {
            content: (0, tool_js_1.formatUntrustedData)(header, JSON.stringify(data, null, 2)),
        };
    }
    async listProcessors(projectId, workspaceName, responseFormat, limit, pageNum) {
        ...
        const format = responseFormat ?? "concise";
        const processors = format === "concise"
            ? data.results.map((p) => ({
                name: p.name,
                state: p.state,
                tier: p.tier,
            ...
            : data.results;
        return {
            content: (0, tool_js_1.formatUntrustedData)(`Found ${data.results.length} processor(s) in workspace '${workspaceName}':`, JSON.stringify(processors, null, 2)),
        };
    }
    async inspectProcessor(projectId, workspaceName, processorName) {
        const data = await this.apiClient.getStreamProcessor({
            params: { path: { groupId: projectId, tenantName: workspaceName, processorName } },
        });
        return {
            content: (0, tool_js_1.formatUntrustedData)(`Processor '${processorName}' in workspace '${workspaceName}':`, JSON.stringify(data, null, 2)),
        };
    }
    async diagnoseProcessor(projectId, workspaceName, processorName) {
        ...
        return {
            content: (0, tool_js_1.formatUntrustedData)(`Diagnostic report for processor '${processorName}' in workspace '${workspaceName}':`, sections.join("\n\n")),
        };
    }
    async getNetworking(projectId, cloudProvider, region) {
        ...
        if (cloudProvider && region) {
            try {
                ...
                sections.push(`## Account Details (${cloudProvider}/${region})\n${JSON.stringify(accountDetails, null, 2)}`);
            }
            catch {
        ...
        return {
            content: (0, tool_js_1.formatUntrustedData)("Streams networking details:", sections.join("\n\n")),
        };
    }
}
exports.StreamsDiscoverTool = StreamsDiscoverTool;
StreamsDiscoverTool.toolName = "atlas-streams-discover";
StreamsDiscoverTool.operationType = "read";
