...
exports.StreamsTeardownTool = void 0;
...
const TeardownResource = zod_1.z.enum(["processor", "connection", "workspace", "privatelink", "peering"]);
class StreamsTeardownTool extends streamsToolBase_js_1.StreamsToolBase {
    constructor() {
        super(...arguments);
        this.description = "Delete Atlas Stream Processing resources. " +
        ...
        this.argsShape = {
            ...
            resource: TeardownResource.describe("What to delete. 'processor': stop first recommended. 'connection': ensure no processor references it. " +
            ...
            resourceName: zod_1.z.string().optional().describe("Name or ID of the specific resource to delete."),
        };
    }
    getConfirmationMessage(args) {
        switch (args.resource) {
            case "workspace": {
                ...
                return (`You are about to delete workspace '${workspace}'. ` +
            ...
            case "processor": {
                ...
                return (`You are about to delete processor '${name}' from workspace '${workspace}'. ` +
            ...
            case "connection": {
                const workspace = this.requireWorkspaceName(args);
                const name = this.requireResourceName(args);
                return (`You are about to delete connection '${name}' from workspace '${workspace}'. ` +
    ...
    async deleteConnection(args) {
        ...
        try {
            ...
            const referencingProcessors = (processors?.results ?? []).filter((p) => {
                const referencedNames = streamsToolBase_js_1.StreamsToolBase.extractConnectionNames(p.pipeline ?? []);
                ...
                if (runningOnes.length > 0) {
                    ...
                    return {
                        content: [
                            {
                                type: "text",
                                text: `Warning: Connection '${name}' is referenced by running processor(s): ${names}. ` +
        ...
        return {
            content: [
                {
                    type: "text",
                    text: `Connection '${name}' deletion initiated in workspace '${workspace}'. ` +
    ...
    async deleteWorkspace(args) {
        ...
        try {
            ...
            const connectionCount = connectionsResult.status === "fulfilled" ? (connectionsResult.value?.results?.length ?? 0) : 0;
            const processorCount = processorsResult.status === "fulfilled" ? (processorsResult.value?.results?.length ?? 0) : 0;
        ...
        return {
            content: [
                {
                    type: "text",
                    text: `Workspace '${workspace}' deletion initiated.${impactNote} ` +
...
exports.StreamsTeardownTool = StreamsTeardownTool;
StreamsTeardownTool.toolName = "atlas-streams-teardown";
StreamsTeardownTool.operationType = "delete";
