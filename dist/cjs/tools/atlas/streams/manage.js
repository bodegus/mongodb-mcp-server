...
exports.StreamsManageTool = void 0;
...
const ManageAction = zod_1.z.enum([
    "start-processor",
    "stop-processor",
    "modify-processor",
    "update-workspace",
...
class StreamsManageTool extends streamsToolBase_js_1.StreamsToolBase {
    constructor() {
        super(...arguments);
        this.description = "Manage Atlas Stream Processing resources: start/stop processors, modify pipelines, update configurations. " +
        ...
        this.argsShape = {
            ...
            workspaceName: streamsArgs_js_1.StreamsArgs.workspaceName().describe("Workspace name containing the resource to manage."),
            action: ManageAction.describe("Action to perform. One of: " +
            ...
            tier: zod_1.z
                ...
                .optional()
                .describe("Override processing tier for this run. " +
            ...
            resumeFromCheckpoint: zod_1.z
                .boolean()
                .optional()
                .describe("Resume from last checkpoint on start. Default: true. " +
                "Set false to reprocess from beginning (drops accumulated window state). Only for 'start-processor'."),
            startAtOperationTime: zod_1.z
                .string()
                .optional()
            ...
            pipeline: zod_1.z
                ...
                .optional()
                .describe("New pipeline stages as an array of objects, e.g. " +
            ...
            newName: zod_1.z.string().optional().describe("Rename processor. Only for 'modify-processor'."),
            // update-workspace options
            newRegion: zod_1.z
                .string()
                .optional()
                .describe("New region for workspace. Only for 'update-workspace'. Use Atlas region names (e.g. AWS: 'VIRGINIA_USA', Azure: 'eastus2', GCP: 'US_CENTRAL1')."),
            newTier: zod_1.z
                .enum(["SP2", "SP5", "SP10", "SP30", "SP50"])
                .optional()
                .describe("New default tier for workspace. Only for 'update-workspace'."),
            // update-connection options
            connectionConfig: streamsArgs_js_1.ConnectionConfig.optional().describe("Updated connection configuration. Only for 'update-connection'. " +
    ...
    getConfirmationMessage(args) {
        switch (args.action) {
            case "start-processor": {
                ...
                const checkpointWarning = args.resumeFromCheckpoint === false
                    ? ` WARNING: resumeFromCheckpoint is false — all accumulated window state will be permanently lost.`
                    : "";
                return (`You are about to start processor '${name}' in workspace '${args.workspaceName}'. ` +
    ...
    async startProcessor(args) {
                ...
                if (maxTier && tierOrder.indexOf(args.tier) > tierOrder.indexOf(maxTier)) {
                    return {
                        content: [
                            {
                                type: "text",
                                text: `Cannot start processor with tier '${args.tier}' — workspace '${args.workspaceName}' has a maximum tier of '${maxTier}'.\n\n` +
                                    `Options:\n` +
        ...
        const hasStartOptions = args.tier !== undefined ||
            args.resumeFromCheckpoint !== undefined ||
            args.startAtOperationTime !== undefined;
        if (hasStartOptions) {
            const startBody = {};
            if (args.tier !== undefined)
                startBody.tier = args.tier;
            if (args.resumeFromCheckpoint !== undefined)
                startBody.resumeFromCheckpoint = args.resumeFromCheckpoint;
            if (args.startAtOperationTime !== undefined)
                startBody.startAtOperationTime = args.startAtOperationTime;
        ...
        const checkpointNote = args.resumeFromCheckpoint === false
        ...
        return {
            content: [
                {
                    type: "text",
                    text: `Processor '${name}' started in workspace '${args.workspaceName}'.${checkpointNote}\n\n` +
    ...
    async stopProcessor(args) {
        ...
        try {
            ...
            this.session.logger.debug({
                id: index_js_1.LogId.streamsProcessorStateLookupFailure,
    ...
    async modifyProcessor(args) {
        ...
        const body = {};
        if (args.pipeline)
            body.pipeline = args.pipeline;
        if (args.newName)
            body.name = args.newName;
        if (args.dlq)
            body.options = { dlq: args.dlq };
        ...
        return {
            content: [
                {
                    type: "text",
                    text: `Processor '${name}' modified (changed: ${changes}).\n\n` +
            ...
            if (!cloudProvider) {
                return {
                    content: [
                        {
                            type: "text",
                            text: "Unable to update workspace region: the current workspace does not specify a cloud provider. " +
        ...
        if (args.newRegion &&
            ...
            return {
                content: [
                    {
                        type: "text",
                        text: `Failed to update workspace region to '${args.newRegion}'. ` +
    ...
    async updateConnection(args) {
        ...
        const normalizedConfig = streamsArgs_js_1.ConnectionConfig.parse(args.connectionConfig);
    ...
    async acceptPeering(args) {
        if (!args.peeringId)
            throw new Error("peeringId is required to accept a VPC peering connection.");
        if (!args.requesterAccountId)
            throw new Error("requesterAccountId is required to accept VPC peering.");
        if (!args.requesterVpcId)
            throw new Error("requesterVpcId is required to accept VPC peering.");
    ...
    async rejectPeering(args) {
        if (!args.peeringId)
            throw new Error("peeringId is required to reject a VPC peering connection.");
...
exports.StreamsManageTool = StreamsManageTool;
StreamsManageTool.toolName = "atlas-streams-manage";
StreamsManageTool.operationType = "update";
