...
const promptForConnectionString = async (config) => {
    ...
    let connectionString = await (0, prompts_1.password)({
...
const promptForServiceAccountSecret = async () => {
    return await (0, prompts_1.password)({ message: "Enter your Atlas Service Account Secret (press enter to skip):", mask: true });
};
...
const runSetup = async (config) => {
    try {
        ...
        const availablePrompts = getAvailablePrompts(connectionString, serviceAccountId, serviceAccountSecret, hasDocker);
