...
export class StreamsDiscoverTool extends StreamsToolBase {
    constructor() {
        ...
        this.argsShape = {
            projectId: AtlasArgs.projectId().describe("Atlas project ID. Use atlas-list-projects to find project IDs if not available."),
    ...
    async listWorkspaces(projectId, responseFormat, limit, pageNum) {
        ...
        const workspaces = format === "concise"
            ...
            : data.results;
        return {
            content: formatUntrustedData(`Found ${data.results.length} workspace(s) (total: ${data.totalCount ?? data.results.length}):`, JSON.stringify(workspaces, null, 2)),
        };
    }
    ...
    async listConnections(projectId, workspaceName, responseFormat, limit, pageNum) {
        ...
        const connections = format === "concise"
            ...
            : data.results;
        return {
            content: formatUntrustedData(`Found ${data.results.length} connection(s) in workspace '${workspaceName}':`, JSON.stringify(connections, null, 2)),
        };
    }
    ...
    async listProcessors(projectId, workspaceName, responseFormat, limit, pageNum) {
        ...
        const processors = format === "concise"
            ...
            : data.results;
        return {
            content: formatUntrustedData(`Found ${data.results.length} processor(s) in workspace '${workspaceName}':`, JSON.stringify(processors, null, 2)),
        };
    }
    async inspectProcessor(projectId, workspaceName, processorName) {
        ...
        return {
            content: formatUntrustedData(`Processor '${processorName}' in workspace '${workspaceName}':`, JSON.stringify(data, null, 2)),
        };
    }
    async diagnoseProcessor(projectId, workspaceName, processorName) {
        ...
        return {
            content: formatUntrustedData(`Diagnostic report for processor '${processorName}' in workspace '${workspaceName}':`, sections.join("\n\n")),
        };
    }
