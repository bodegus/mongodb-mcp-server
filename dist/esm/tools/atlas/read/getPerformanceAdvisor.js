...
import { getSuggestedIndexes, getDropIndexSuggestions, getSchemaAdvice, getSlowQueries, DEFAULT_SLOW_QUERY_LOGS_LIMIT, SUGGESTED_INDEXES_COPY, SLOW_QUERY_LOGS_COPY, } from "../../../common/atlas/performanceAdvisorUtils.js";
...
export class GetPerformanceAdvisorTool extends AtlasToolBase {
    constructor() {
        super(...arguments);
        this.description = `Get MongoDB Atlas performance advisor recommendations and suggestions, which includes the operations: suggested indexes, drop index suggestions, schema suggestions, and a sample of the most recent (max ${DEFAULT_SLOW_QUERY_LOGS_LIMIT}) slow query logs`;
        this.argsShape = {
            projectId: AtlasArgs.projectId().describe("Atlas project ID to get performance advisor recommendations. The project ID is a hexadecimal identifier of 24 characters. If the user has only specified the name, use the `atlas-list-projects` tool to retrieve the user's projects with their ids."),
        ...
        try {
            const [suggestedIndexesResult, dropIndexSuggestionsResult, slowQueryLogsResult, schemaSuggestionsResult] = await Promise.allSettled([
                ...
                operations.includes("slowQueryLogs")
                    ? getSlowQueries(this.apiClient, projectId, clusterName, since ? new Date(since) : undefined, namespaces)
                    : Promise.resolve(undefined),
...
GetPerformanceAdvisorTool.toolName = "atlas-get-performance-advisor";
