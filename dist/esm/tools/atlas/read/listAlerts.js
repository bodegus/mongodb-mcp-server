...
export class ListAlertsTool extends AtlasToolBase {
    ...
    async execute({ projectId, status, limit, pageNum, }) {
        ...
        return {
            content: formatUntrustedData(`Found ${alerts.length} alerts with status "${status}" in project ${projectId} (total: ${data.totalCount ?? alerts.length})`, JSON.stringify(alerts)),
        };
    }
}
