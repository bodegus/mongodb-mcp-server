 ...
 * constructor
 */
export declare abstract class ToolBase<TUserConfig extends UserConfig = UserConfig, TContext = unknown, TMetrics extends DefaultMetrics = DefaultMetrics> {
    /**
     * The unique name of this tool.
     *
     * Must be unique across all tools in the server.
     ...
     * ```
     */
    protected abstract execute(args: ToolArgs<typeof this.argsShape>, context: ToolExecutionContext): Promise<CallToolResult>;
    /** This is used internally by the server to invoke the tool. It can also be run manually to call the tool directly. */
    invoke(args: ToolArgs<typeof this.argsShape>, context: ToolExecutionContext): Promise<CallToolResult>;
    /**
    ...
    constructor({ name, category, operationType, session, config, telemetry, elicitation, metrics, uiRegistry, context, }: ToolConstructorParams<TUserConfig, TContext, TMetrics>);
