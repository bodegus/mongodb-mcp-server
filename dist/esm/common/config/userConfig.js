import { z } from "zod";
import { commaSeparatedToArray, getExportsPath, getLogPath, oneWayOverride, onlyLowerThanBaseValueOverride, onlyStricterLogLevelOverride, onlySubsetOfBaseValueOverride, parseBoolean, } from "./configUtils.js";
...
export const configRegistry = z.registry();
const ServerConfigSchema = z.object({
    ...
    connectionString: z
        ...
        .register(configRegistry, { isSecret: true, overrideBehavior: "not-allowed" }),
    loggers: z
        .preprocess((val) => commaSeparatedToArray(val), z.array(z.enum(["stderr", "disk", "mcp"])))
        .check(z.minLength(1, "Cannot be an empty array"), z.refine((val) => new Set(val).size === val.length, {
    ...
    disabledTools: z
        .preprocess((val) => commaSeparatedToArray(val), z.array(z.string()))
        .default([])
    ...
    voyageApiKey: z
        ...
        .register(configRegistry, { isSecret: true, overrideBehavior: "not-allowed" }),
    previewFeatures: z
        .preprocess((val) => commaSeparatedToArray(val), z.array(z.enum(previewFeatureValues)))
        .default([])
    ...
    monitoringServerFeatures: z
        .preprocess((val) => commaSeparatedToArray(val), z.array(z.enum(monitoringServerFeatureValues)))
